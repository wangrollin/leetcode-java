package com.wangrollin.leetcode.n0_normal.p1300.p1370.problem1379;
...
public class Solution1 {
    ...
    public final TreeNode getTargetCopy(final TreeNode original, final TreeNode cloned, final TreeNode target) {
        ...
        String way = findTheWay(original, target, "");

        TreeNode result = cloned;

        for (char ch : way.toCharArray()) {
            if (ch == 'l') {
                result = result.left;
            } else {
                result = result.right;
    ...
    private String findTheWay(TreeNode node, TreeNode target, String curWay) {
        ...
        if (node.left != null) {
            if ((theWay = findTheWay(node.left, target, curWay + "l")) != null) {
                return theWay;
            }
        }
        if (node.right != null) {
            theWay = findTheWay(node.right, target, curWay + "r");
        }
