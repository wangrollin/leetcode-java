package com.wangrollin.leetcode.n0_normal.p1400.p1490.problem1491;
...
public class Solution1 {
    ...
    public double average(int[] salary) {
        ...
        return (double) (Arrays.stream(salary).sum() -
                Arrays.stream(salary).min().getAsInt() -
                Arrays.stream(salary).max().getAsInt()) / (salary.length - 2);
    }
}
