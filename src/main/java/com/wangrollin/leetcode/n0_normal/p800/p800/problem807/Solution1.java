package com.wangrollin.leetcode.n0_normal.p800.p800.problem807;
...
public class Solution1 {
    ...
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        ...
        int rowMax[] =
                Arrays.stream(grid)
                        .mapToInt(arr -> Arrays.stream(arr).max().getAsInt())
                        .toArray();
        ...
        for (int row = 0; row < grid.length; row++) {
            for (int col = 0; col < grid.length; col++) {
                maxIncrease += Math.min(rowMax[row], colMax[col]) - grid[row][col];
            }
        }
        return maxIncrease;
    }
