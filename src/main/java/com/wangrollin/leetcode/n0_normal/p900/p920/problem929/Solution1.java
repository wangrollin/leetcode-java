package com.wangrollin.leetcode.n0_normal.p900.p920.problem929;
...
public class Solution1 {
    ...
    public int numUniqueEmails(String[] emails) {
        ...
        Arrays.stream(emails).forEach(email -> {
            ...
            String front = email.substring(0, email.indexOf('@')).replace(".", "");

            int indexOfPlus = -1;
            if ((indexOfPlus = front.indexOf('+')) != -1) {
                front = front.substring(0, indexOfPlus);
            }

            emailSet.add(front + email.substring(email.indexOf('@')));
